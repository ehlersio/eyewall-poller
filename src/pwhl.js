...
Only reference the two teams named above and the numbers given -- no player names, no invented stats or games. Plain text only, no markdown, no bullet points.`;
  ...
  if (url.pathname === '/pwhl/player-game-log') {
    ...
    return cachedJson(env, `pwhl:player-game-log:${playerId}:${seasonId}`, 24 * 3600, async () => {
      const [skaters, goalies] = await Promise.all([
        sbRows(`${SB_URL}/rest/v1/pwhl_skater_game_box?player_id=eq.${playerId}&season_id=eq.${seasonId}&order=game_id.asc`),
        sbRows(`${SB_URL}/rest/v1/pwhl_goalie_game_box?player_id=eq.${playerId}&season_id=eq.${seasonId}&order=game_id.asc`),
      ]);
      if (skaters instanceof Response) return skaters;
      if (goalies instanceof Response) return goalies;
      return { skaters, goalies };
  ...
  if (url.pathname === '/pwhl/player/landing') {
    ...
    return cachedJson(env, `pwhl:player:landing:${playerId}:${seasonParam || 'latest'}`, 3600, async () => {
      const playerRows = await sbRows(`${SB_URL}/rest/v1/pwhl_players?player_id=eq.${playerId}&select=*`);
      ...
      const stats = (await sbRowsOr(`${SB_URL}/rest/v1/${statsTable}?${statsQuery}`, []))[0] || {};
  ...
  if (url.pathname === '/pwhl/player/percentiles') {
    ...
    return cachedJson(env, `pwhl:player:percentiles:${playerId}:${seasonQuery || 'latest'}:${seasonType}`, 3600, async () => {
      ...
      let rows;
      try {
        rows = await sbRows(`${SB_URL}/rest/v1/pwhl_player_seasons?${statsQuery}`);
      } catch (e) {
        return errorJson(502, { error: e.message });
      }
      if (rows instanceof Response) return rows;
  ...
  if (url.pathname === '/pwhl/goalie/percentiles') {
    ...
    return cachedJson(env, `pwhl:goalie:percentiles:${playerId}:${seasonQuery || 'latest'}:${seasonType}`, 3600, async () => {
      ...
      let rows;
      try {
        rows = await sbRows(`${SB_URL}/rest/v1/pwhl_goalie_seasons?${statsQuery}`);
      } catch (e) {
        return errorJson(502, { error: e.message });
      }
      if (rows instanceof Response) return rows;
  ...
  if (url.pathname === '/pwhl/transactions') {
    const season = await seasonParam(url, env);
    return cachedJson(env, `pwhl:transactions:${season}`, 24 * 3600, async () => {
      const htRes = await fetch(
      ...
      try {
        ...
        return errorJson(502, { error: 'transactions parse failed', detail: e.message });
      }
...
Write a 2-3 sentence scouting report highlighting their strengths, style of play, and impact this season. Be specific and use the stats given above — do not invent awards, games, injuries, teammates, or any other detail not in this data. If a stat is missing ('—'), skip it or say it isn't available rather than guessing a number. Do not use generic filler phrases. Write in plain text, no markdown.`;
  ...
  if (url.pathname === '/pwhl/player-shots') {
    ...
    // 6hr cache
    return cachedJson(env, `pwhl:pshots:${playerId}:${season}`, 3600 * 6, async () => {
      const rows = await sbRows(
  ...
  if (url.pathname === '/pwhl/goalie-shots') {
    ...
    // 6hr cache
    return cachedJson(env, `pwhl:gshots:${goalieId}:${season}`, 3600 * 6, async () => {
      const rows = await sbRows(
  ...
  if (url.pathname.startsWith('/pwhl/live/')) {
    ...
    // KV TTL: 1hr once final, else 60s.
    ...
    return cachedJson(env, `pwhl:live:${gameId}`, ttl, async () => {
