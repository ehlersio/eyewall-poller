...
| `nhl:elo-win-prob:{HOME}:{AWAY}:{0|1 neutral}` | 1hr | One matchup's Elo home-win probability from `team_elo_ratings` (updated nightly). A failed read is not cached |
...
| `GET` | `/elo/win-prob?home=&away=&neutral=` | One matchup's Elo home-win probability — the number the game preview's win bar shows and the same one `eyewall-pipeline`'s `win_probs.py` logs each morning for the scorecard: `team_elo_ratings` + the Elo expected score, with the 35-point home advantage unless `neutral=1`. Response: `{ home, away, neutral, homeWinProb, ratings: { home, away } }`; a team with no rating uses 1500. home/away must be different 2–3 letter abbreviations (400 otherwise). KV-cached 1hr; a failed read returns `unavailable: true`, not cached. `/prediction/analyze` now applies the same neutral-site rule (it used to give the home side the advantage at neutral sites too). |
