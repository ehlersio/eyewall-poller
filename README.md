...
**Pre-game predictions (2026-09)** — `/game-predictions` also filters on `locale` (`game_predictions` got its `locale` column in `eyewall-pipeline`'s `docs/session_locale_predictions.sql`; its KV key gets a `:fr` suffix for French only). The on-demand AI prediction routes — `/prediction/analyze` (both the in-season branch and `buildPreseasonFallback()`), `/pwhl/prediction`, `/ahl/prediction` and `/echl/prediction` — take `?locale=fr` too: `shared.js`'s `localizePrompt()` appends `FRENCH_INSTRUCTION` (a copy of the pipeline's `STICKS_SYSTEM_PROMPT_FR_ADDENDUM` — keep the two in step) and `localeKeySuffix()` gives French its own `:fr` KV key. English keys are unchanged, so existing caches and the frontend's `/cache/prediction:{gameId}:{team}` lookups keep working. The other on-demand AI routes (`/summary/narrative`, `/pwhl/scout`, the head-to-head narratives, `/draft/analyze`) are still English-only.
...
- **No Corsi/Fenwick/PDO anywhere in either file, on purpose** — there's no shot-attempts data source beyond shots-on-goal for either league. `/ahl/prediction` and `/echl/prediction` started as ports of `/pwhl/prediction`'s heuristic without the Corsi term; since 2026-09 their win % is Elo instead (see the `/ahl/prediction` row below).
...
- **No league-scoped `/ahl/cache/bust` or `/echl/cache/bust` route exists**, unlike PWHL's `POST /pwhl/cache/bust`, which clears one team's whole set of caches in a single call. The generic `POST /cache/bust?key=` (2026-09) covers the same ground one key at a time, so a stale AHL/ECHL entry no longer needs `wrangler kv key delete` by hand — what's still missing is only the convenience of clearing a team's several keys at once.
...
- `src/__tests__/worker-routes.test.js` — the dispatcher (`/config/seasons`, `/config/seasons/pwhl-types`, `/config/seasons/ahl-seasons` / `/echl-seasons`, `/pwhl/*` vs. everything-else routing).
...
- `src/__tests__/hockeytech-leagues.characterization.test.js` (added 2026-09, ahead of merging `ahl.js`/`echl.js` into one shared implementation) — runs every AHL and ECHL route, `pollAHL`/`pollECHL`, and `fetchAHLNews`/`fetchECHLNews` against one shared fixture set (keyed by table suffix and HockeyTech view, so the same fixtures drive both leagues) and snapshots what each does: response status/body, every upstream request (URL, `Range`, HockeyTech `Referer`, the AI prompt), and every KV write with its TTL. Also asserts that repeat calls are served from cache, missing params return 400, and a Supabase outage returns 502. The date is faked to an in-season January evening so output is deterministic. **The snapshots are the refactor's safety net:** a snapshot diff means behavior changed — explain it in the PR rather than updating the snapshot blindly. The poll tests also pin the game-over push: sent once for a game the poll followed live, never for one already final the first time it's seen.
- `src/__tests__/pwhl-routes.characterization.test.js` (added 2026-09, ahead of moving `pwhl.js` onto the shared route helpers) — the same snapshot approach for every `pwhl.js` route, `pollPWHL` and `fetchPWHLNews`: response status/body, every upstream request, every KV write with its TTL, and the Supabase-down, HockeyTech/AI-down and missing-param paths. Covers the 11 routes nothing tested before (`/players`, `/shots`, `/schedule`, `/roster`, `/lastgame`, `/pbp`, `/salaries`, `/league-players`, `/player-shots`, `GET /news`, `/today`). Same rule: a snapshot diff is a behavior change to explain in the PR.
- `src/__tests__/nhl-routes.characterization.test.js` (added 2026-09, ahead of moving `nhl.js` onto the shared route helpers) — the same snapshot approach for every `handleNHL` route, plus the background-fetch routes (`/news`, the current-season `/schedule`, `/cache/:key`) and push subscribe/unsubscribe: response status/body, every upstream request, every KV write with its TTL, and the Supabase-down, NHL-API/AI-down and missing-param paths. Covers the 16 routes nothing tested before (`/goalie-shots`, `/team-lines`, `/game-xg`, `/game-log`, `/xg-trend`, `/power-rankings`, `/game-predictions`, `/game-summary`, `/player-scouting`, `/team-skaters`, `/players-list`, `/special-teams`, `/draft/rankings`, `/draft/picks`, `/draft/order`, `/player/landing`). `poll()`'s push notifications stay covered by `nhl-routes.test.js`.
- `src/__tests__/shared-route-helpers.test.js` (added 2026-09) — `shared.js`'s route helpers: `cachedJson` (cache hit, miss-then-store with its TTL or a TTL computed from the data, a `Response` from the builder returned uncached), `sbRows`/`sbRowsOr` (auth plus extra headers, the 502 on a failed read, the optional-read fallback), and `sbError()`'s "Supabase error" default.
- `src/__tests__/pwhl-poll.test.js` (added 2026-09) — `pollPWHL`'s game-over push, same two cases as the AHL/ECHL poll tests above. Until 2026-09 that push never fired for any HockeyTech league, because the poll only handed live games to the per-game handler.
...
| `nhl:projected-lines:{team}` | 1hr | A team's projected lines and D pairs for its next game (Supabase `projected_lines`, written nightly by `eyewall-pipeline`'s `projected_lines.py`). Neither a failed read nor an empty result (no projection yet) is cached |
...
| `pp_units:{season}` | 4hr | PP/PK unit rosters for one season — the cron re-reads Supabase only once this lapses; `/pp-units/refresh` forces a re-read (e.g. right after a pipeline run). Season-scoped as of 2026-09 (was a single flat `pp_units:all`): the shot map can display a season other than the current one, and one key can only hold one season's units. Only the current season is kept warm by the cron; any other season is filled on first request |
...
| `ahl:prediction:elo:{gameId}` / `echl:prediction:elo:{gameId}` | 30min | Elo win probability + AI narrative (`:elo` added 2026-09 so point-split predictions cached before the switch expired instead of being served) |
...
| `GET` | `/config/seasons/ahl-seasons`, `/config/seasons/echl-seasons` | Every season the league's HockeyTech feed knows about, current and historical: `[{seasonId, seasonName, seasonType, startYear, startDate, endDate}]`, from the same 6hr KV-cached feed as `/config/seasons` (`config:season:{ahl,echl}:seasons`). Read by the pipeline's `season_lookup.py` for a season's type and its game-log date window. `502` when the feed is unavailable (never a guess), same as `/config/seasons/pwhl-types`. |
...
| `GET` | `/nhl/goal-replay/:gameId/:eventId` | One NHL goal's player and puck tracking (NHL EDGE) for the app's goal replay, Video | Tracking (added 2026-09, `src/goalReplay.js`). The goal's `pptReplayUrl` is read from the game center `landing` feed (never built from the ids, and only ever fetched from `wsr.nhle.com`), then fetched with a browser User-Agent -- the file answers 403 without one, and browsers can't fetch it cross-origin. Response: `{ available: true, gameId, eventId, hz: 10, goalFrame, attacksRight, scorerId, teams: {home, away}, players: {trackId: {playerId, number, team}}, frames: [{ puck: [x, y] | null, at: {trackId: [x, y]} }] }`, positions in play-by-play feet (`x = inches_x/12 - 100`, `y = 42.5 - inches_y/12`, checked against 40 goals' shot spots), ~30-60 KB against ~200-250 KB raw. `goalFrame` is the frame the puck is in the net, same rules as `eyewall-pipeline`'s `goal_of_week.py`. `404 { available: false }` when there's no replay (none before 2023-24; not published yet mid-game). KV-cached (`nhl:goal-replay:v1:{gameId}:{eventId}`) a year once the game is final, 5 min while live (files are rewritten at the final horn), 60 s when missing. Ids are validated (400 otherwise). |
...
| `GET` | `/playoff-odds?team=&season=` | A team's simulated playoff odds from Supabase's `playoff_odds` / `playoff_odds_game_impacts` (`eyewall-pipeline`'s nightly `playoff_odds.py`: the rest of the regular season simulated from team Elo ratings). Without `season`, the season of the team's most recent run. Response: `{ team, season, runDate, stale, latest, history, nextGames }` — `latest` = that run's row (`playoff_pct`, `division_pct`, `proj_points`, `points_p10`/`points_p90`, `current_points`, `games_played`, `games_remaining`, `elo_rating`, `sims`, and `change`: `{ prev_run_date, prev_pct, delta, contributions: [{ game_id, home, away, winner, delta }], residual }`, why it moved since the previous run — null on a season's first run); `history` = `[{ run_date, playoff_pct }]` for that season, oldest first; `division` = the team's whole division from that same run, so `division_pct` reads as a standing rather than a bare number: `{ name, rank, of, teams: [{ team, divisionPct, playoffPct, projPoints }] }`, ranked by division odds (division membership comes from the cached NHL standings' `divisionName`; `null` when those are unavailable); `nextGames` = the team's odds under each result of the next game-day's games, `[{ game_id, game_date, home, away, ifHomeWins, ifAwayWins, own, swing }]`, own games first, then up to 2 other games that swing its odds by ≥ 0.5 pts (`summarizeNextGames()`, `src/playoffOdds.js`); `stale` = no run for 3+ days (season over, or the nightly stopped). Before the season's first run `latest` is null. team (2–3 letters) and season (8 digits) are validated (400 otherwise). KV-cached 1hr; a failed read returns `unavailable: true` and is **not** cached, nor is an empty result. |
...
| `GET` | `/projected-lines?team=` | A team's projected forward lines and D pairs for its next game, from Supabase's `projected_lines` (`eyewall-pipeline`'s nightly `projected_lines.py`: last game's pairings in-season, pooled preseason pairings before the team's first game; see that repo's `docs/projected_lines_backtest_results.md` for accuracy). Response: `{ team, basis: 'last_game' \| 'preseason' \| null, basisGameId, basisGames, generatedAt, lines, pairs }`, where `lines`/`pairs` are ranked `{ rank, players: [{ id, name, pos, filled }] }` with forwards ordered L-C-R, and `filled` marks a replacement for a player who was in the basis lineup (`summarizeProjectedLines()`, `src/projectedLines.js`). `basis: null` with empty lists means there's no projection yet. Different from `/team-lines`, which is the season's most-used units with xGF%. `team` must be 2–3 letters (400 otherwise). KV-cached 1hr; a failed read returns `unavailable: true` and is **not** cached, nor is an empty result. |
| `GET` | `/nhl/today` | The scoreboard's games — today's when there are any, otherwise the next day that has some. Same normalized shape as `/pwhl/today`/`/ahl/today`/`/echl/today` (session102), sourced from the live NHL API rather than a Supabase table (NHL has no `nhl_game_log`-equivalent; the API already returns real team abbrevs/scores). **Not `score/now`:** that endpoint is not "today" out of season — on 2026-09-15 it served Sept 29's regular-season opener and skipped the Sept 19 preseason games entirely. The route asks for today's Eastern date (`score/{date}`) and, when that date is empty, walks forward via `schedule/{date}`'s `gameWeek` (which does include preseason) to the next date with games. Each game carries `gameDate`, `startTimeUTC` and `gameType` so the client can say "Today" only when it means it, plus, while live, `period`/`periodType`/`clock`/`inIntermission`, and `endedIn` (`OT`/`SO`) once final. `broadcasts` lists the game's TV networks (`[{network, market, countryCode}]`, national first, then away, then home; from the NHL feed's `tvBroadcasts`). The clock is a snapshot, not a ticking one (60s KV TTL, matching `poll()`'s own cadence). |
| `GET` | `/special-teams?season=` | PP/PK unit rosters for every team that has them, as `{ ABBR: { PP: { 1: [ids], 2: [ids] }, PK: { … } } }`, from Supabase's `special_teams_units` (`eyewall-pipeline`'s `special_teams.py` infers them from shift + shot events). `season` optional, defaulting to the resolved current season — the shot map passes whichever season it is displaying, since it falls back to the last played one between the schedule being published and opening night. Returns `{}` rather than an error for a season with no rows, which is the normal state for a season that hasn't been played. KV-cached 4hr per season (`pp_units:{season}`) |
...
| `POST` | `/live-activity/start-token` | "Follow my team's games" (iOS 17.2+): the app's ActivityKit push-to-start token for the user's team, `{token, team, enabled, locale}`. Kept in `la:start:{TEAM}` as `{token, locale}` (90 days, re-sent on every app launch); `la:startteam:{token}` records the team so changing favorites moves it, and `enabled: false` removes it. When that team's game is live, `poll()`'s `startLiveActivities()` sends each token one APNs `event: start` push per game (tracked in `la:started:{gameId}`), carrying the `GameActivityAttributes` fields, including each team's display colour (`LA_TEAM_COLORS`, kept in sync with eyewall-analytics' `teamConfig.js`), plus an en/fr alert. The Live Activity appears even if the app is closed. The app then reports the new activity's update token to `/live-activity/register`, and updates carry on as above |
...
| `GET` | `/pp-units/refresh?season=` | Refresh PP/PK unit data — `season` optional, defaults to the resolved current season |
...
| `GET` | `/pwhl/player/landing?id=&season=` | Single player's identity + one season's stat line, merged (pwhl_players + pwhl_player_seasons/pwhl_goalie_seasons). Powers `PWHLPlayerPopup`'s self-fetch-by-id. `season` pins the stat line to that `season_id`, regular or playoffs (a season_id belongs to one season type, so no type filter is applied); omitted, falls back to the most recent regular-season row. |
...
| `GET` | `/pwhl/today?season=` | Today's games, or the next day that has games, with live status. Each game carries `gameDate`, HockeyTech's own `statusDetail` text (a start time before puck drop), `period` while live and `endedIn` (`OT`/`SO`) once final. This feed has no game clock. |
...
| `GET` | `/pwhl/prediction?gameId=&force=` | Win probability + AI narrative for an upcoming game (Session 51). Win % is Elo since 2026-09 (`pwhl_team_elo_ratings`, written by eyewall-pipeline's `hockeytech_elo.py`; +35 home advantage, a team with no rating at 1500), replacing the point-split heuristic, which backtested worse than a coin flip and gave the home team 0% whenever stats tied. Response carries `winModel: 'elo'`; KV key is `pwhl:prediction:elo:{gameId}`. Corsi/PP% still feed the AI prompt. `corsiForPct` prefers 5v5-filtered shot-attempt share, falling back to all-situations if the 5v5 column isn't populated yet (Session 53, same preference order as `/prediction/analyze`) — check `corsiCaveat` for which one a given response used |
...
| `GET` | `/ahl/prediction?gameId=&force=` | Win probability + AI narrative for an upcoming game. Win % is Elo (2026-09): `ahl_team_elo_ratings` (eyewall-pipeline's `hockeytech_elo.py`) with +35 home advantage, a team with no rating at 1500 -- replacing the point-split heuristic, which backtested worse than a coin flip and gave the home team 0% whenever stats tied (`docs/hockeytech_elo_backtest_results.md` in eyewall-pipeline). Response carries `winModel: 'elo'`. The AI prompt's stats context is unchanged |
...
| `GET` | `/ahl/today?season=` | Today's games, or the next day that has games, with live status. Each game carries `gameDate` and HockeyTech's own `statusDetail` text. Unlike PWHL, this league's game log stores no period or clock. |
...
`pollAHL` (built by `hockeytech.js`, not an HTTP route) runs from `worker.js`'s per-minute `scheduled()` alongside NHL/PWHL/ECHL's own poll functions — push notifications for game start, period start, goals (with hat-trick detection), power plays, pulled goalies and the final score, gated on `env.VAPID_PRIVATE_KEY` existing and an Oct–June active-season window. The final-score push goes out once, and only for a game the poll followed live — a game that had already ended the first time it's seen (e.g. right after a deploy) gets none. (Before 2026-09 it never fired at all, for PWHL either: the poll only handed live games to the per-game handler.)
...
| `GET` | `/echl/prediction?gameId=&force=` | Win probability + AI narrative — Elo from `echl_team_elo_ratings`, same as `/ahl/prediction` |
