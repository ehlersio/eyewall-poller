...
| `milestones:latest:{sport}` | 1hr | Cheapest "is there anything new" check for the frontend's Milestones read-state badge (Phase 2, Session 92) — `{latestId, gameDate}` only, not the full feed. |
...
| `GET` | `/trivia/today?sport=&team=` | All three trivia tiers (easy/medium/hard) for one sport in a single response, `team` optional (adds that team's medium-tier question; omit for easy/hard only) — Daily Trivia (Phase 2, Session 92). Reads `trivia_questions`, nightly-populated by the pipeline's `trivia_questions.py`. Correct answer/explanation are included in the response (not split into a post-answer reveal step) — accepted for a low-stakes trivia feature. |
...
| `GET` | `/milestones/latest?sport=` | Same badge purpose as `/news/latest`, for Milestones — its own minimal `limit=1` query since no reusable unfiltered KV entry exists for that table (the real `/milestones` route's key always includes `team`+`limit`). `gameDate` is date-granularity, not a timestamp — `milestones` has no insertion-time column, accepted as good enough for a boolean seen/unseen badge. |
