...
The routes still without tests are NHL plain read-proxy routes (parse params → cache check → `sbRows()`/fetch → cache write → JSON), the same shape already covered here and mechanical to extend. AHL/ECHL and PWHL routes are all covered by the characterization suites above.
